        ...
        <div className='mx-auto max-w-7xl px-6'>
          <div className='flex flex-col gap-12'>
            <div className='reveal-anim active'>
              <span className='text-[10px] font-black uppercase tracking-[0.3em] text-[#001f3f] dark:text-brand mb-4 inline-block'>
                Direct Access
              </span>
              <h2 className='text-4xl md:text-6xl font-black text-neutral-900 dark:text-white leading-tight'>
                Explore Our <br />{' '}
                <span className='text-[#001f3f] dark:text-brand'>
                  Learning Ecosystem
                </span>
              </h2>
            </div>
              ...
              <div className='grid grid-cols-1 md:grid-cols-4 gap-8 items-start'>
                <div className='md:col-span-1 filter-sidebar space-y-10'>
                  <div className='space-y-6'>
                    <p className='text-[10px] font-black uppercase tracking-widest text-brand-200 opacity-50'>
                      Categories
                    </p>
                    <div className='flex flex-col gap-3'>
                      {coreCategories.map((cat) => (
                        <button
                          ...
                          className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all w-fit text-left ${
                            activeCategory === cat
                  ...
                  {loading ? (
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
                      {[1, 2, 3, 4, 5, 6].map((i) => (
                        <div
                          key={i}
                          className='h-80 rounded-3xl bg-neutral-200 dark:bg-neutral-800 animate-pulse'
                        ></div>
                      ))}
                    </div>
                  ) : (
                    <>
                      {error && (
                        <div className='p-4 bg-red-50 text-red-500 rounded-2xl mb-8 font-medium'>
                          Error: {error}
                        </div>
                      )}
